...
module V1
  class PostsCreateTest < ActionDispatch::IntegrationTest
    test 'create action should return correct status on request' do
      post '/api/posts', headers: { 'Authorization': authenticated_header, 'Accept': 'application/v1' },
        params: { post: { title: 'test_test', body: 'Hello, I am Vasya', username: 'vasya' } }
    ...
    test 'create action should create record with correct values' do
      ...
      username = 'Dart Vader'
      post '/api/posts', headers: { 'Authorization': authenticated_header, 'Accept': 'application/v1' },
        params: { post: { title: title, body: body, username: username } }
    ...
    test 'create action should increase number of post records' do
      assert_difference 'Post.count', 1 do
        post '/api/posts', headers: { 'Authorization': authenticated_header, 'Accept': 'application/v1' },
          params: { post: { title: 'star_wars', body: 'May the Force be with you', username: 'Obi-Wan' } }
      end
    end
  end
end
